<template>
    <div class="aftersale">
        ...
        <van-tabs>
            <van-tab title="售后申请">
                <van-panel v-for="(order,index) in orders" :key="index" :title="'订单号：'+order.ordercode"
                           style="margin-top:10px;">
                    <div>
                        <product-card v-for="(product,i) in order.products" :key="i" :product='product'>
                            <div class="van-panel_product_footer">
                                <van-button size="small" type="danger" tag="a"
                                            :href="'#/user/aftersale/apply?id='+product.id">申请售后
                                </van-button>
                            </div>
                        </product-card>
                    </div>
            ...
            <van-tab title="申请记录">
                <van-panel v-for="(order,index) in orders" :key="index" :title="'服务单号：'+order.ordercode"
                           style="margin-top:10px;">
                    <div>
                        <product-card v-for="(product,i) in order.products" :key="i" :product='product'/>
                    </div>
                    <div>
                        <van-cell title="已完成" :to="'/user/aftersale/detail?ordercode='+order.ordercode" is-link
                                  label="服务已完成，感谢您对京东的支持" style="background-color: #f8f8f8;"/>
                    </div>
                </van-panel>
